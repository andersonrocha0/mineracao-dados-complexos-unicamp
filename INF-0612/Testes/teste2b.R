...
#a
notasDasProvas <- data.frame(ids, p1, p2, p3, p4)
notasDasProvas

#b
notasDasProvas$p1AoQuadrado <- notasDasProvas$p1 ^ 2
notasDasProvas$p2AoQuadrado <- notasDasProvas$p2 ^ 2
notasDasProvas$p3AoQuadrado <- notasDasProvas$p3 ^ 2
notasDasProvas$p4AoQuadrado <- notasDasProvas$p4 ^ 2
notasDasProvas$somaDasNotasAoQuadrado <- rowSums(notasDasProvas[, 6:9])
notasDasProvas$menorNotaAoQuadrado <- rowMins(notasDasProvas[, 6:9])
medquad <- round(sqrt((notasDasProvas$somaDasNotasAoQuadrado - notasDasProvas$menorNotaAoQuadrado)/3), digits = 2)
medquad

#c
mp1 <- colMeans(notasDasProvas[,2:5])[1]
mp1
mp2 <- colMeans(notasDasProvas[,2:5])[2]
mp2
mp3 <- colMeans(notasDasProvas[,2:5])[3]
mp3
mp4 <- colMeans(notasDasProvas[,2:5])[4]
mp4

#d
dp1 <- sd(notasDasProvas[,2])
dp1
dp2 <- sd(notasDasProvas[,3])
dp2
dp3 <- sd(notasDasProvas[,4])
dp3
dp4 <- sd(notasDasProvas[,5])
dp4

#e
notasDasProvas$medQuad = medquad
notasrank <- notasDasProvas$ids[order(notasDasProvas$medQuad, decreasing = T)]
notasrank